...
<%
	...
	AssetEntry assetEntry = AssetEntryLocalServiceUtil
		.getEntry(GuestbookEntry.class.getName(), entry.getEntryId());
		...
		<liferay-ui:panel collapsible="<%=true%>" extended="<%=true%>"
			...
			<%
				Discussion discussion = CommentManagerUtil.getDiscussion(
								user.getUserId(), scopeGroupId, GuestbookEntry.class.getName(),
			...
			<c:if test="<%=discussion != null%>">
				<h2>
					<strong> <liferay-ui:message
							...
							key='<%=(discussion.getDiscussionCommentsCount() == 1) ? "x-comment"
									: "x-comments"%>' /></strong>
				</h2>
