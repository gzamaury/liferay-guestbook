...
public class GuestbookEntryLocalServiceImpl extends GuestbookEntryLocalServiceBaseImpl {
	...
	public GuestbookEntry addGuestbookEntry(long userId, long guestbookId, String name,
		...
		AssetEntry assetEntry = assetEntryLocalService.updateEntry(userId, groupId,
			entry.getCreateDate(), entry.getModifiedDate(), GuestbookEntry.class.getName(),
			entryId, entry.getUuid(), 0, serviceContext.getAssetCategoryIds(),
			serviceContext.getAssetTagNames(), true, true, null, null, null, null,
			ContentTypes.TEXT_HTML, entry.getMessage(), null, null, null, null, 0, 0, null);
	...
	public GuestbookEntry updateGuestbookEntry(long userId, long guestbookId, long entryId,
		...
		AssetEntry assetEntry = assetEntryLocalService.updateEntry(userId,
			...
			entry.getCreateDate(), null, null, null, ContentTypes.TEXT_HTML, entry.getMessage(),
	...
	protected void validate(String name, String email, String entry) throws PortalException {
		...
		if (Validator.isNull(name)) {
			throw new GuestbookEntryNameException("Name is empty.");
		}

		if (!Validator.isEmailAddress(email)) {
			throw new GuestbookEntryEmailException("Email is wrong: " + email);
		}

		if (Validator.isNull(entry)) {
			throw new GuestbookEntryMessageException("Message is empty.");
		}
	}
